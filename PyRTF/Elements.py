...
def _get_jpg_dimensions( fin ):
	...
	M_SOF0   = chr( 0xC0 ).encode('latin-1')	#	/* Start Of Frame N */
	M_SOF1   = chr( 0xC1 ).encode('latin-1')	#	/* N indicates which compression process */
	M_SOF2   = chr( 0xC2 ).encode('latin-1')	#	/* Only SOF0-SOF2 are now in common use */
	M_SOF3   = chr( 0xC3 ).encode('latin-1')	#
	M_SOF5   = chr( 0xC5 ).encode('latin-1')	#	/* NB: codes C4 and CC are NOT SOF markers */
	M_SOF6   = chr( 0xC6 ).encode('latin-1')	#
	M_SOF7   = chr( 0xC7 ).encode('latin-1')	#
	M_SOF9   = chr( 0xC9 ).encode('latin-1')	#
	M_SOF10  = chr( 0xCA ).encode('latin-1')	#
	M_SOF11  = chr( 0xCB ).encode('latin-1')	#
	M_SOF13  = chr( 0xCD ).encode('latin-1')	#
	M_SOF14  = chr( 0xCE ).encode('latin-1')	#
	M_SOF15  = chr( 0xCF ).encode('latin-1')	#
	M_SOI    = chr( 0xD8 ).encode('latin-1')	#	/* Start Of Image (beginning of datastream) */
	M_EOI    = chr( 0xD9 ).encode('latin-1')  #	/* End Of Image (end of datastream) */

	M_FF = chr( 0xFF ).encode('latin-1')
...
def _get_png_dimensions( data ) :
	...
	new_data = [data[18], data[19], data[22], data[23]]
	for i, d in enumerate(new_data):
		if isinstance(d, str):
			new_data[i] = ord(d)

	width  = (new_data[0] * 256) + (new_data[1])
	height = (new_data[2] * 256) + (new_data[3])
	return width, height
...
class Image( RawCode ) :
	...
	def __init__( self, file_name, **kwargs ) :
		...
		fin.seek( 0, 0 )
		data = []
		image = hexlify( fin.read() ).decode('latin-1')
